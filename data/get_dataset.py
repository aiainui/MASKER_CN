...
from data.base_dataset import NewsDataset, ReviewDataset, IMDBDataset, SST2Dataset, FoodDataset, \
    ReutersDataset, MSRvidDataset, ImagesDataset, MSRparDataset, HeadlinesDataset, Weibo_6clfDataset, \
    emo_8411_10clf_4thDataset
...
def get_base_dataset(data_name, tokenizer, split_ratio=1.0, seed=0, test_only=False, remain=False):
    ...
    elif data_name == 'weibo_6clf':
        dataset = Weibo_6clfDataset(tokenizer, test_only=test_only)
    elif data_name == 'emo_8411_10clf_4th':
        dataset = emo_8411_10clf_4thDataset(tokenizer, test_only=test_only)
    else:
...
def get_keyword(args, dataset, tokenizer, keyword_type, keyword_per_class):
    if keyword_type == 'tfidf':
        ...
        state_dict = torch.load(os.path.join(CKPT_PATH, dataset.data_name, args.attn_model_path), map_location='cpu')
