...
class BaseDataset(metaclass=ABCMeta):
    def __init__(self, data_name, total_class, tokenizer,
        ...
        self.split_ratio = split_ratio # 选择的类别/总的类别=split_ratio
        self.seed = seed
        self.remain = remain
...
class Weibo_6clfDataset(BaseDataset):
    def __init__(self, tokenizer, test_only=False):
        super(Weibo_6clfDataset, self).__init__('weibo_6clf', 6, tokenizer, test_only=test_only)

    def _preprocess(self):
        print('Pre-processing weibo_6clf dataset...')
        train_dataset = self._load_dataset('train')
...
class emo_8411_10clf_4thDataset(BaseDataset):
    def __init__(self, tokenizer, test_only=False):
        super(emo_8411_10clf_4thDataset, self).__init__('emo_8411_10clf_4th', 10, tokenizer, test_only=test_only)

    def _preprocess(self):
        print('Pre-processing emo_8411_10clf_4th dataset...')
        train_dataset = self._load_dataset('train')
        test_dataset = self._load_dataset('test')
    ...
    def _load_dataset(self, mode='train', raw_text=False):
        ...
        for k, v in df.iterrows():
            ...
            toks.append(v['text'])
            toks.append(v['label'])
            # text  = tokenize(self.tokenizer, 'a')
            # label = torch.tensor(int(0)).long()
            if raw_text:
                text = toks[0][:512]
            else:
                text = tokenize(self.tokenizer, toks[0][:512])

            # if len(text) > 512:
            #     print('EEEEEEEEEEEEEEEE',text,toks[0] )
            label = torch.tensor(int(toks[1])).long()
            # print(text.shape)
            inputs.append(text)
            labels.append(label)
